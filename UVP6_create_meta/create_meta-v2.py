...
#now the script saves the txt file in the meta folder of your project
#
# author : Joelle Habib, 2024
# Modified by Ema Coicaud-T, 09/2025
...
for input_file_path in config_txt_string:
    # Try multiple encoding if one failed
    encodings_to_try = ['utf-8', 'latin-1', 'cp1252']
    lines = None

    for enc in encodings_to_try:
        try:
            with open(input_file_path, 'r', encoding=enc) as input_file:
                lines = input_file.readlines()
            print(f"✅ File {input_file_path} read with the encoding {enc}")
            break
        except UnicodeDecodeError:
            print(f"⚠️ Unreadable {input_file_path} with {enc}, we try an other one...")
    
    if lines is None:
        print(f"Error : unreadable {input_file_path} with standard encodings.")
        continue
    ...
    for line in lines:
        ...
        if "Pixel_Size = " in line:
            pixel_size_value = float(line.split('=')[1].strip()) / 1000
            ...
            Aa_value = float(line.split('=')[1].strip()) / 1_000_000

cruise_value= "UNKNOWN"
...
# Find the folder meta in the projet
meta_path = os.path.join(path_uvp_project, "meta")
os.makedirs(meta_path, exist_ok=True)  # create the folder if it does'nt exist
